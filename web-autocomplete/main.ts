declare const EMAIL_DATA: string;
(async () => {
...
const empty_array = new Array<Contact>(); // not to be modified in place

const receivers = new Array<Contact>();
...
function onInput(this: HTMLInputElement, _?: KeyboardEvent) {
	...
	for (let i = 0; i < length; i++) {
		...
		const item = result_items[i] as HTMLElement;
...
input.oninput = onInput as never;
onInput.call(input);

function renderHover(i: number) {
...
input.onkeydown = function (this: HTMLInputElement, ev: KeyboardEvent) {
	switch (ev.key) {
		...
		case ",": case "Enter": {
			...
			if (isFullyFormedEmail(this.value))
				selectContact(tree.contacts.find(c => c.email === this.value) ?? { first_name: "", last_name: "", email: this.value, timestamp: 0 });
...
for (let i = 0, { length } = result_items; i < length; i++) {
	const item = result_items[i] as HTMLLIElement;
	item.onclick = function (this: HTMLLIElement, _: MouseEvent) {
		selectContact(query_results[i]!);
	} as never;

	item.onmouseover = function(this: HTMLLIElement, _: MouseEvent) {
		selected_result = i;
		renderHover(selected_result);
	} as never;
}
...
function isFullyFormedEmail(email: string) {
	let i = email.indexOf("@");
